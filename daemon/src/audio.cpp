...
const char* AudioEngine::kNodeName = "mic-effects";
...
bool AudioEngine::startImpl(const std::string& label, std::string* err) {
  ...
  loop_ = pw_thread_loop_new("mic-effects-audio", nullptr);
  ...
  core_ = pw_context_connect(context_, pw_properties_new(PW_KEY_APP_NAME, "Microphone Effects",
                                                        PW_KEY_APP_ID, "whoiscalebbrown.mic-effects",
                                                        "mic-effects.client", "true", nullptr), 0);
  ...
  pw_properties* props = pw_properties_new(
      ...
      PW_KEY_NODE_LINK_GROUP, "mic-effects",
      PW_KEY_DEVICE_API, "mic-effects-server",
      nullptr);
...
bool AudioEngine::openCapture(std::string* err) {
  ...
  pw_properties* props = pw_properties_new(
      ...
      PW_KEY_MEDIA_CATEGORY, "Capture",
      PW_KEY_MEDIA_ROLE, "Communication",
      PW_KEY_NODE_NAME, "mic-effects-input",
...
bool AudioEngine::openListen(std::string* err) {
  ...
  pw_properties* props = pw_properties_new(
      ...
      PW_KEY_NODE_NAME, "mic-effects-listen",
...
bool AudioEngine::openNudge(const std::string& target, std::string* err) {
  ...
  pw_properties* props = pw_properties_new(
      ...
      PW_KEY_MEDIA_CATEGORY, "Capture",
      PW_KEY_MEDIA_ROLE, "Communication",
      PW_KEY_NODE_NAME, "mic-effects-headset",
...
std::string AudioEngine::maintain(double now) {
  ...
  if (!healthy) {
    okSince_ = -1;
    if (source_) {
      fprintf(stderr, "mic-effects-server: mic stream lost, reconnecting in %.0f s\n", backoff_);
      stop();
    ...
    std::string err;
    if (start(label_, &err)) { fprintf(stderr, "mic-effects-server: microphone node back\n"); return status_; }
    status_ = err;
    retryAt_ = now + backoff_;
    backoff_ = std::min(backoff_ * 2, 30.0);
    return status_;
  ...
  if (capture_ && (!want || retarget)) {
    ...
    closeCapture();
    pw_thread_loop_unlock(loop_);
    if (!want) fprintf(stderr, "mic-effects-server: microphone released\n");
  }
  if (want && !capture_ && now >= captureRetryAt_) {
    ...
    fprintf(stderr, "mic-effects-server: microphone open (%s)\n", cur.empty() ? "default source" : cur.c_str());
  }
  ...
  if (!bt.empty() && !nudge_ && now >= nudgeRetryAt_) {
    ...
    pw_thread_loop_unlock(loop_);
    if (ok) fprintf(stderr, "mic-effects-server: asking for the headset profile on %s\n", bt.c_str());
  ...
  if (listen_ && wantListen && listenLost_.exchange(false)) {
    ...
    fprintf(stderr, "mic-effects-server: listen stream lost, retrying in %.0f s\n", listenRetryAt_ - now);
  }
...
void AudioEngine::onGlobal(unsigned id, const char* type, const spa_dict* props) {
  ...
  if (name.rfind("mic-effects", 0) == 0 || name.rfind("camera-effects", 0) == 0) return; // virtual effects mics
  MicSourceInfo s;
  s.id = id;
  s.name = name;
...
void AudioEngine::onCoreError(unsigned id, int, int res, const char* message) {
  ...
  fprintf(stderr, "mic-effects-server: pipewire core error: %s (%d)\n", message ? message : "", res);
  failed_ = true;
}

void AudioEngine::onPlaybackState(int old, int state, const char* error) {
  if (error) fprintf(stderr, "mic-effects-server: microphone node error: %s\n", error);
...
void AudioEngine::onListenState(int old, int state, const char* error) {
  if (error) fprintf(stderr, "mic-effects-server: microphone listen error: %s\n", error);
...
void AudioEngine::onCaptureState(int old, int state, const char* error) {
  if (error) fprintf(stderr, "mic-effects-server: microphone input error: %s\n", error);
